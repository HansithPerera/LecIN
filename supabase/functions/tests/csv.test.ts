...
const testClientCreation = async () => {
  ...
  if (newAttendanceError) {
    throw new Error(
      "Failed to insert new attendance: " + newAttendanceError.message,
    );
  }
};
...
Deno.test(
  "Export Attendance CSV Test",
  { sanitizeResources: false },
  testExportAttendanceCsv,
);
Deno.test(
  "Unauthorized with Anon Key Test",
  { sanitizeResources: false },
  testUnauthorizedWithAnon,
);
Deno.test(
  "Success with Service Key Test",
  { sanitizeResources: false },
  testSuccessWithServiceKey,
);
